...
module.exports = class CurrentTasksList extends React.Component {
    ...
    render() {
        if(this.state.todos.length > 0){
            return <div className="current-task-list" ><TodoList todos={this.state.todos}/></div>
        } else {
            return <div className="current-task-list" ><div className="error-empty">Aucune tâche active</div></div>
        }
    }
}